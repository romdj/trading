...
echo 'querying fields' $fields
echo 'querying symbols' $symbols
echo 'final query' "$API_ENDPOINT&fields=$fields&symbols=$symbols"

# results=$(curl --silent "$API_ENDPOINT&fields=$fields&symbols=$symbols" \
#   | jq '.quoteResponse .result')

# query () {
#   echo "querying: " $(curl --silent "$API_ENDPOINT&fields=$fields&symbols=$symbols" \
#   | jq '.quoteResponse .result')
#   echo "jq -r .[] | select (.symbol == \"$1\") | .$2"
