...
tickers.forEach(ticker => {
  yahooFinance.historical({
    ...
    if (err) {
      ...
      // console.log(`found quotes for ${ticker}, length: ${quotes.length}, size: ${JSON.stringify(quotes).length}`);
      // appendFileSync(resultDataPath, JSON.stringify(quotes), 'utf8');
      formatDataDynamo(quotes).forEach(formatQuotes => dispatchDynamo(formatQuotes));
    }
